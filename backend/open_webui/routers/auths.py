...
async def signin(request: Request, response: Response, form_data: SigninForm):
    ...
    # Log login attempt
    try:
        from open_webui.models.login_logs import LoginLogs
        login_type = "password"
        if WEBUI_AUTH_TRUSTED_EMAIL_HEADER:
            login_type = "trusted_header"
        elif WEBUI_AUTH == False:
            login_type = "no_auth"
        
        LoginLogs.insert_login_log(
            id=str(uuid.uuid4()),
            user_id=user.id if user else None,
            user_email=form_data.email.lower() if hasattr(form_data, 'email') else "unknown",
        ...
        log.error(f"Failed to log login attempt: {str(e)}")
