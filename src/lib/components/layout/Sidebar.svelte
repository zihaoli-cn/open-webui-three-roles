...
>
	...
	>
		...
		{#if $user?.role === 'admin' || $user?.role === 'system_admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools}
			<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				<a
					class="grow flex items-center space-x-3 rounded-lg px-2 py-[7px] hover:bg-gray-100 dark:hover:bg-gray-900 transition"
					href="/workspace"
		...
		{#if $user?.role === 'audit_admin'}
			<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				...
				>
					...
					<div class="flex self-center translate-y-[0.5px]">
						<div class=" self-center font-medium text-sm font-primary">安全审计</div>
					</div>
				</a>
			</div>
		{/if}
